...
export function recoveryReasonForQaStep(workflows: RecoveryWorkflow[], sessions: RecoverySession[]): string | null {
  const issues = workflows.flatMap((workflow) => workflow.issues);
  if (issues.some((issue) => hasAnyLabel(issue, ["taskix:env-blocked"]))) return "QA is blocked by the local validation environment. Fix the environment or provide a usable preview before retrying QA.";
  ...
  if (issues.some((issue) => (issue.prUrl || issue.prState?.toUpperCase() === "OPEN") && !hasAnyLabel(issue, ["taskix:need-qa", "taskix:qa-running", "qa-passed", "taskix:qa-passed", "qa-failed", "taskix:qa-failed", "taskix:spec-blocked", "taskix:env-blocked", "taskix:ready-to-merge"]))) {
