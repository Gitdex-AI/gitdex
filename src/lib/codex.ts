...
- Treat submitted developer tests as the primary reusable verification asset. Before browser/manual checks, run the focused tests listed by Developer and assess whether those tests cover each acceptance criterion.
- Do not edit product code or test code in the temporary QA worktree as the way to make the PR pass. If tests are missing, stale, or do not cover the acceptance criteria, fail QA as an implementation issue and ask Developer to add or update focused tests in the PR.
- After developer fixes or rebases, prefer rerunning the recorded test commands. Repeat focused browser smoke only when conflict resolution or UI wiring changed, or when the acceptance criteria cannot reasonably be automated.
